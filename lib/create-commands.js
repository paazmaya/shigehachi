 ...
 */
const compare = function compare(diffPicture, prevPicture, currPicture, settings) {
  return [
    'compare',
    '-metric',
    settings.metric,
 ...
 */
const composite = function composite(diffPicture, prevPicture, currPicture, settings) {
  let append = '-composite';
  if (settings.longDiffName) {
    append += '-' + settings.compose;
  }
  const compositeFile = diffPicture.replace(MATCH_PNG, append + '.png');
 ...
 */
const negate = function negate(diffPicture) {
  const negateFile = diffPicture.replace(MATCH_PNG, '-negate.png');
